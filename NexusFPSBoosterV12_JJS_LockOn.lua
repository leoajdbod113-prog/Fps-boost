...
--==================================================
-- AIM LOCK - JJS STYLE LOCK-ON
--==================================================
-- Reworked to mimic the observed Jujutsu Shenanigans Lock-On behavior:
-- acquire a nearby opponent from the camera direction, keep the same target,
-- smoothly turn the camera to keep that target in view, and avoid aimbot-like
-- head snapping/prediction.

local LOCKON_RANGE = 75
local LOCKON_ACQUIRE_FOV = 0.62
local LOCKON_BREAK_FOV = 1.25
local LOCKON_TURN_SPEED = math.rad(240)
local LOCKON_SMOOTH = 0.20
local LOCKON_REACQUIRE_DELAY = 0.20
local LOCKON_LOST_GRACE = 0.35
local LOCKON_TARGET_HEIGHT = 1.65
local LOCKON_MIN_TARGET_ANGLE = math.rad(55)
local LOCKON_ALLOW_OCCLUDED = false

local AimTarget = nil
local AimTargetPlayer = nil
local AimLostAt = 0
local AimLastAcquire = 0
local AimLastTargetPosition = nil
local LockOnCursor = nil
...
local function GetLockOnPart(character)
	...
	-- Body-centered rather than head-centered: closer to a camera lock-on.
	return character:FindFirstChild("UpperTorso")
		or character:FindFirstChild("Torso")
		or character:FindFirstChild("HumanoidRootPart")
end

local function GetLockOnCandidate(player)
	if not player or player == Player then return nil end
	local character = player.Character
	if not character then return nil end
	...
	if distance > LOCKON_RANGE or distance < 2 then return nil end

	local direction = offset.Unit
	local angle = math.acos(math.clamp(Camera.CFrame.LookVector:Dot(direction), -1, 1))
	if angle > LOCKON_MIN_TARGET_ANGLE then return nil end
	...
	if not LOCKON_ALLOW_OCCLUDED then
...
local function FindLockOnTarget()
	...
	local center = Vector2.new(Camera.ViewportSize.X * 0.5, Camera.ViewportSize.Y * 0.5)
	local screenScale = math.max(Camera.ViewportSize.X, Camera.ViewportSize.Y) * LOCKON_ACQUIRE_FOV

	for _, player in ipairs(Players:GetPlayers()) do
		local candidate = GetLockOnCandidate(player)
		if candidate then
			local screenWeight = candidate.ScreenDistance / screenScale
			local distanceWeight = candidate.Distance / LOCKON_RANGE
			local angleWeight = candidate.Angle / LOCKON_MIN_TARGET_ANGLE

			-- Camera direction is dominant, then screen position, then distance.
			local score = angleWeight * 0.55 + screenWeight * 0.30 + distanceWeight * 0.15
...
local function IsLockedTargetValid()
	if not AimTargetPlayer then return false end
	local candidate = GetLockOnCandidate(AimTargetPlayer)
	if not candidate then return false end

	-- Once locked, give the target a much wider cone than acquisition.
	-- This keeps the lock from constantly dropping during dashes.
	if candidate.Angle > LOCKON_BREAK_FOV then return false end
	if candidate.Distance > LOCKON_RANGE * 1.18 then return false end

	AimTarget = candidate
	return true
end

local function ResetAimMemory()
	AimTarget = nil
	AimTargetPlayer = nil
	AimLostAt = 0
	AimLastAcquire = 0
	AimLastTargetPosition = nil
end
...
local function AcquireLockOn()
	...
	AimLastAcquire = os.clock()
	AimLostAt = 0
	AimLastTargetPosition = target.Root.Position
	return true
end

local function ReleaseLockOn()
	ResetAimMemory()
end

local function GetLockOnLookPosition(target)
	if not target or not target.Root then return nil end

	-- Keep the camera around the opponent's upper body. No velocity prediction.
	-- This is intentionally different from an aimbot that predicts a head hitbox.
	local root = target.Root
	local pos = root.Position + Vector3.new(0, LOCKON_TARGET_HEIGHT, 0)

	-- Smooth the target point very slightly so rapid animation changes don't
	-- introduce camera micro-jitter.
	if AimLastTargetPosition then
		pos = AimLastTargetPosition:Lerp(pos, 0.55)
	end
	AimLastTargetPosition = pos
	return pos
end
...
local function UpdateAimLock(dt)
	...
	if not AimTargetPlayer then
		if now - AimLastAcquire >= LOCKON_REACQUIRE_DELAY then
			AcquireLockOn()
		end
	else
		if not IsLockedTargetValid() then
			if AimLostAt == 0 then AimLostAt = now end
			if now - AimLostAt >= LOCKON_LOST_GRACE and now - AimLastAcquire >= LOCKON_REACQUIRE_DELAY then
				ReleaseLockOn()
				AcquireLockOn()
			end
		end
	...
	local targetPosition = GetLockOnLookPosition(AimTarget)
	if not targetPosition then return end

	local cameraPosition = Camera.CFrame.Position
	local currentLook = Camera.CFrame.LookVector
	local desiredLook = (targetPosition - cameraPosition)
	if desiredLook.Magnitude < 0.001 then return end
	...
	-- JJS-like behavior: smooth follow with a capped rotation rate rather
	-- than snapping the camera directly onto the target every frame.
	local frame = math.max(dt or (1/60), 1/240)
	local rateAlpha = math.min(1, (LOCKON_TURN_SPEED * frame) / angle)
	local smoothAlpha = 1 - math.pow(1 - LOCKON_SMOOTH, frame * 60)
	local alpha = math.min(rateAlpha, smoothAlpha)

	local blendedLook = currentLook:Lerp(desiredLook, math.clamp(alpha, 0, 1)).Unit
	Camera.CFrame = CFrame.lookAt(cameraPosition, cameraPosition + blendedLook)
end
