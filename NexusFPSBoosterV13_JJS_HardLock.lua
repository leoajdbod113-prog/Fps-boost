...
--==================================================
-- AIM LOCK - JJS HARD LOCK
--==================================================

local LOCKON_RANGE = 120
local LOCKON_ACQUIRE_FOV = 0.62
local LOCKON_SMOOTH = 0.42
local LOCKON_TURN_SPEED = math.rad(540)
...
local function GetJJSCharacter(player)
...
local function GetJJSAimPart(character)
	if not character then return nil end
...
local function ClearJJSLock()
	AimTarget = nil
	AimTargetPlayer = nil
	AimLastTargetPosition = nil
end

local function GetJJSCharacterData(player)
	local character = GetJJSCharacter(player)
	if not character then return nil end

	local root = character:FindFirstChild("HumanoidRootPart")
	local part = GetJJSAimPart(character)
	local humanoid = character:FindFirstChildOfClass("Humanoid")
	...
	local maxScreenDistance =
		math.max(Camera.ViewportSize.X, Camera.ViewportSize.Y) * LOCKON_ACQUIRE_FOV
...
local function AcquireJJSLock()
	...
	for _, player in ipairs(Players:GetPlayers()) do
		local candidate = GetJJSCharacterData(player)
		...
		if candidate then
			...
			local score = screenWeight * 0.75 + distanceWeight * 0.25
	...
	AimTargetPlayer = best.Player
	AimTarget = best
	AimLastTargetPosition = best.Root.Position

	return true
end

local function RefreshJJSLockParts()
	if not AimTargetPlayer then
		ClearJJSLock()
		return false
	end

	local character = GetJJSCharacter(AimTargetPlayer)
...
local function EnableJJSLock()
	ClearJJSLock()
	return AcquireJJSLock()
end

local function DisableJJSLock()
	ClearJJSLock()
end

local function UpdateJJSLockOn(dt)
	local Camera = Workspace.CurrentCamera
	...
	-- HARD LOCK:
	-- once acquired, this function never searches for another player.
	-- If the locked player disappears/dies, the lock is simply cleared.
	if not AimTargetPlayer then
		return
	end

	if not RefreshJJSLockParts() then
		return
	end

	local root = AimTarget.Root
	local targetPosition = root.Position + Vector3.new(0, LOCKON_TARGET_HEIGHT, 0)

	-- No velocity prediction. The camera follows the target's current position.
	if AimLastTargetPosition then
		local followAlpha = 1 - math.pow(1 - LOCKON_SMOOTH, math.max(dt or (1 / 60), 1 / 240) * 60)
		AimLastTargetPosition = AimLastTargetPosition:Lerp(targetPosition, math.clamp(followAlpha, 0, 1))
	else
		AimLastTargetPosition = targetPosition
	end

	targetPosition = AimLastTargetPosition

	local cameraPosition = Camera.CFrame.Position
	local desiredVector = targetPosition - cameraPosition
	if desiredVector.Magnitude < 0.001 then return end

	local desiredLook = desiredVector.Unit
	...
	local turnAlpha = math.min(1, (LOCKON_TURN_SPEED * frame) / angle)
	local smoothAlpha = 1 - math.pow(1 - LOCKON_SMOOTH, frame * 60)
	local alpha = math.clamp(math.max(turnAlpha, smoothAlpha), 0, 1)

	local blendedLook = currentLook:Lerp(desiredLook, alpha)
	if blendedLook.Magnitude < 0.001 then return end

	Camera.CFrame = CFrame.lookAt(
		cameraPosition,
		cameraPosition + blendedLook.Unit
	)
end
