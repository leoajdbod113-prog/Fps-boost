...
local AimTarget = nil
local KokusenEnabled = false
local KokusenWindow = false
local KokusenCooldown = false
...
if SavedConfig then
	...
	AimLockEnabled = SavedConfig.AimLockEnabled or false
	KokusenEnabled = SavedConfig.KokusenEnabled or false
end
...
local function GetAimPart(model)
	if not model or model == Player.Character then return nil end
...
local function GetTargetHumanoid(part)
	if not part then return nil end
	local model = part:FindFirstAncestorOfClass("Model")
	return model and model:FindFirstChildOfClass("Humanoid")
end

local function IsValidAimTarget(part)
	if not part or not part.Parent or not part:IsDescendantOf(Workspace) then
		return false
	end
	local humanoid = GetTargetHumanoid(part)
...
local function FindAimTarget()
	...
	for _, otherPlayer in ipairs(Players:GetPlayers()) do
		if otherPlayer ~= Player then
			consider(otherPlayer.Character)
		end
	end

	-- Apenas modelos diretamente no Workspace: evita percorrer todos os
	-- descendants a cada frame, o que podia causar travamentos/atrasos.
...
local function UpdateAimLock()
	...
	if not IsValidAimTarget(AimTarget) then
		AimTarget = FindAimTarget()
	else
		local screenPos, onScreen = Camera:WorldToViewportPoint(AimTarget.Position)
		local viewport = Camera.ViewportSize
		local center = Vector2.new(viewport.X * 0.5, viewport.Y * 0.5)
		local screenDistance = (Vector2.new(screenPos.X, screenPos.Y) - center).Magnitude

		if not onScreen or screenPos.Z <= 0 or screenDistance > AIM_FOV * 1.6 then
			AimTarget = FindAimTarget()
		end
	end

	if not IsValidAimTarget(AimTarget) then return end

	local cameraPosition = Camera.CFrame.Position
	local desired = CFrame.lookAt(cameraPosition, AimTarget.Position)
...
--==================================================
-- KOKUSEN / BLACK FLASH TIMING HELPER
--==================================================

local KokusenTiming = 0.12
local KokusenStatus = "READY"

local function TriggerKokusenWindow()
	if not KokusenEnabled or KokusenCooldown then return end
	KokusenCooldown = true
	KokusenWindow = true
	KokusenStatus = "TIMING"
	task.delay(KokusenTiming, function()
		KokusenWindow = false
		KokusenStatus = "READY"
	end)
	task.delay(0.35, function()
		KokusenCooldown = false
	end)
end

-- Compatibilidade básica: observa Tools ativadas sem depender de RemoteEvents ofuscados.
local function WatchCharacter(character)
	local function watch(child)
		if child:IsA("Tool") then child.Activated:Connect(TriggerKokusenWindow) end
	end
	for _, child in ipairs(character:GetChildren()) do watch(child) end
	character.ChildAdded:Connect(watch)
end
if Player.Character then WatchCharacter(Player.Character) end
Player.CharacterAdded:Connect(function(character) task.wait(0.25); WatchCharacter(character) end)
...
--==================================================
-- KOKUSEN FLOATING BUTTON
--==================================================

local KokusenButton = Instance.new("TextButton")
KokusenButton.Name = "KokusenButton"
KokusenButton.Size = UDim2.fromOffset(58, 58)
KokusenButton.Position = UDim2.new(1, -82, 0.5, 38)
KokusenButton.BackgroundColor3 = KokusenEnabled and Color3.fromRGB(45, 15, 65) or Color3.fromRGB(20, 15, 25)
KokusenButton.BackgroundTransparency = 0.12
KokusenButton.BorderSizePixel = 0
KokusenButton.Text = "⚡"
KokusenButton.TextSize = 25
KokusenButton.AutoButtonColor = false
KokusenButton.ZIndex = 50
KokusenButton.Parent = Gui

local KokusenCorner = Instance.new("UICorner")
KokusenCorner.CornerRadius = UDim.new(1, 0)
KokusenCorner.Parent = KokusenButton

local KokusenStroke = Instance.new("UIStroke")
KokusenStroke.Color = PURPLE
KokusenStroke.Thickness = 2
KokusenStroke.Transparency = KokusenEnabled and 0 or 0.25
KokusenStroke.Parent = KokusenButton

local KokusenStateLabel = Instance.new("TextLabel")
KokusenStateLabel.Size = UDim2.new(1, 0, 0, 15)
KokusenStateLabel.Position = UDim2.new(0, 0, 1, -16)
KokusenStateLabel.BackgroundTransparency = 1
KokusenStateLabel.Text = KokusenEnabled and "ON" or "OFF"
KokusenStateLabel.TextColor3 = KokusenEnabled and GREEN or RED
KokusenStateLabel.TextSize = 8
KokusenStateLabel.Font = Enum.Font.GothamBold
KokusenStateLabel.ZIndex = 51
KokusenStateLabel.Parent = KokusenButton

local function RefreshKokusenButton()
	KokusenButton.BackgroundColor3 = KokusenEnabled and Color3.fromRGB(45, 15, 65) or Color3.fromRGB(20, 15, 25)
	KokusenStroke.Transparency = KokusenEnabled and 0 or 0.25
	KokusenStateLabel.Text = KokusenEnabled and "ON" or "OFF"
	KokusenStateLabel.TextColor3 = KokusenEnabled and GREEN or RED
...
KokusenButton.MouseButton1Click:Connect(function()
	KokusenEnabled = not KokusenEnabled
	KokusenWindow = false
	KokusenStatus = KokusenEnabled and "READY" or "OFF"
	RefreshKokusenButton()
	PersistConfig()
end)
...
local KokusenTitle = Instance.new("TextLabel")
KokusenTitle.Size = UDim2.new(1, 0, 0, 24)
KokusenTitle.Position = UDim2.fromOffset(0, 355)
KokusenTitle.BackgroundTransparency = 1
KokusenTitle.Text = "KOKUSEN"
KokusenTitle.TextColor3 = PURPLE
KokusenTitle.TextSize = 11
KokusenTitle.Font = Enum.Font.GothamBold
KokusenTitle.TextXAlignment = Enum.TextXAlignment.Left
KokusenTitle.Parent = PlayerPage

local KokusenToggle = Instance.new("TextButton")
KokusenToggle.Size = UDim2.fromOffset(110, 38)
KokusenToggle.Position = UDim2.new(1, -110, 0, 348)
KokusenToggle.BackgroundColor3 = KokusenEnabled and Color3.fromRGB(15, 45, 30) or PANEL2
KokusenToggle.Text = KokusenEnabled and "ON" or "OFF"
KokusenToggle.TextColor3 = KokusenEnabled and GREEN or RED
KokusenToggle.TextSize = 13
KokusenToggle.Font = Enum.Font.GothamBold
KokusenToggle.AutoButtonColor = false
KokusenToggle.Parent = PlayerPage
local KokusenToggleCorner = Instance.new("UICorner")
KokusenToggleCorner.CornerRadius = UDim.new(0, 8)
KokusenToggleCorner.Parent = KokusenToggle
local KokusenToggleStroke = Instance.new("UIStroke")
KokusenToggleStroke.Color = PURPLE
KokusenToggleStroke.Thickness = 1.5
KokusenToggleStroke.Parent = KokusenToggle

local KokusenHint = Instance.new("TextLabel")
KokusenHint.Size = UDim2.new(1, -8, 0, 35)
KokusenHint.Position = UDim2.fromOffset(4, 393)
KokusenHint.BackgroundTransparency = 1
KokusenHint.Text = "Janela de timing: 0.12s • botão fixo"
KokusenHint.TextColor3 = Color3.fromRGB(180, 170, 190)
KokusenHint.TextSize = 9
KokusenHint.Font = Enum.Font.GothamMedium
KokusenHint.TextXAlignment = Enum.TextXAlignment.Center
KokusenHint.Parent = PlayerPage

local function RefreshKokusenControls()
	KokusenToggle.Text = KokusenEnabled and "ON" or "OFF"
	KokusenToggle.TextColor3 = KokusenEnabled and GREEN or RED
	KokusenToggle.BackgroundColor3 = KokusenEnabled and Color3.fromRGB(15, 45, 30) or PANEL2
	RefreshKokusenButton()
end
KokusenToggle.MouseButton1Click:Connect(function()
	KokusenEnabled = not KokusenEnabled
	KokusenWindow = false
	KokusenStatus = KokusenEnabled and "READY" or "OFF"
	RefreshKokusenControls()
	PersistConfig()
end)
