...
local function FindAimTarget()
	...
	for _, otherPlayer in ipairs(Players:GetPlayers()) do
		if otherPlayer ~= Player then
			local part = GetAimPart(otherPlayer.Character)
			if part then
...
local function UpdateAimLock()
	...
	if not AimTarget
		...
		or not AimTarget:IsDescendantOf(Workspace) then
	...
	if AimTarget then
		...
		Camera.CFrame = Camera.CFrame:Lerp(desired, 0.35)
	end
end

RunService:BindToRenderStep("NexusAimLock", Enum.RenderPriority.Camera.Value + 1, UpdateAimLock)
